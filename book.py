...
def reader_book(id):
	...
	error = None
	book = query_db('''select * from books where book_id = ?''', [id], one=True)
	reader = query_db('''select * from borrows where book_id = ? and user_name = ? ''', [id, g.user], one=True)
	count  = query_db('''select count(book_id) from borrows where user_name = ? and date_return = '' ''',
			  [g.user], one = True)
	...
	if request.method == 'POST':
		if reader:
			if reader['date_return'] != '':
				db = get_db()
				db.execute('''update borrows set date_borrow = ?,date_return = '' where book_id = ? and user_name = ?''', [current_time, id, g.user])
				db.execute('''update histroys set status = ? where user_name = ? and book_id = ? ''', ['unreturn', g.user, id])
				db.commit()
			...
			if count[0] == 3:
				...
				db.execute('''insert into borrows (user_name, book_id, date_borrow, \
					date_return) values (?, ?, ?, '') ''', [g.user, id, current_time])
...
@app.route('/reader/book/<id>/return', methods=['POST'])
def reader_book_return(id):
	reader_judge()
	error = None
	book = query_db('''select * from books where book_id = ? ''', [id], one=True)
	reader = query_db('''select * from borrows where book_id = ? and user_name = ? ''', [id, g.user], one=True)
	...
	if reader:
		if reader['date_return'] == '':
			db = get_db()
			db.execute('''update borrows set date_return = ? where book_id = ? and user_name = ?''', [current_time, id, g.user])
			db.execute('''update histroys set status = ? where user_name = ? and book_id = ? ''', ['returned', g.user, id])
			db.commit()
			return redirect(url_for('reader_book', id = id))
		else:
			error = 'The book has already returned.'
	else:
		error = 'The book has already returned.'
	return render_template('reader_book.html', book = book, reader = reader, error = error)
